...
from stubo.cache.queue import String, Queue, get_redis_subordinate
from stubo.cache.backends import RedisCacheBackend, get_redis_main
...
class Cache(object):
    ...
    def get_scenario_key(self, session_name):
        ...
        scenario = self.get_cache_backend()(get_redis_subordinate()).get_raw(key, session_name)
        if not scenario:
            return None
    ...
    def get_all_saved_request_index_data(self):
        ...
        for key in keys:
            ...
            info[scenario_name] = self.get_cache_backend()(main).get_all(key)
        return info
    ...
    def delete_caches(self, scenario_name):
        key = self.scenario_key_name(scenario_name)
        main = get_redis_main()
        deleted_responses = self.get_cache_backend()(main).remove(self.get_response_key(
            scenario_name))
        deleted_requests = self.get_cache_backend()(main).remove(self.get_request_key(
            scenario_name))
        ...
        for k in (self.get_request_index_key(scenario_name),
            ...
            deleted_request_indexes.append(self.get_cache_backend()(main).remove(k))

        session_names = self.get_cache_backend()(main).keys(key)
        ...
        if session_names:
            ...
            for k in session_names:
                deleted_sessions_map += self.get_cache_backend()(main).delete(sessions_key, k)
        deleted_sessions = self.get_cache_backend()(main).remove(key)
    ...
    def set_delay_policy(self, name, data):
        key = self.get_delay_policy_key()
        return self.get_cache_backend()(get_redis_main()).set(key, name, data)
    ...
    def get_request_index_data(self, scenario_name):
        main = get_redis_main()
        return self.get_cache_backend()(main).get_all_raw(self.get_request_index_key(
            scenario_name))
    ...
    def delete_saved_request_index(self, scenario_name, name):
        main = get_redis_main()
        return self.get_cache_backend()(main).delete(self.get_saved_request_index_key(
            scenario_name), name)
    ...
    def delete_session_data(self, scenario_name, session):
        ...
        for _hash in hashes:
            keys = self.get_cache_backend()(main).keys(_hash)
            ...
            if session_keys:
                ...
                for k in session_keys:
                    num_deleted += self.get_cache_backend()(main).delete(_hash, k)
    ...
    def assert_valid_session(self, scenario_name, session_name):
        ...
        sessions_key = '{0}:sessions'.format(self.host)
        scenario_found = self.get_cache_backend()(get_redis_main()).get_raw(sessions_key,
                                                                     session_name)
    ...
    def get_response(self, scenario_name, session_name, response_ids,
        ...
        if num_responses > 1:
            ...
            if not index or index < num_responses:
                index = self.get_cache_backend()(main).incr(request_index_name, request_index_key)
            index -= 1
    ...
    def set_stubo_setting(self, setting, value, all_hosts=False):
        ...
        return self.get_cache_backend()(get_redis_main()).set(key, setting, value)
    ...
    def get_stubo_setting(self, setting=None, all_hosts=False):
        ...
        if not all_hosts:
            key = '{0}:{1}'.format(self.host, key)
        if setting:
            result = self.get_cache_backend()(get_redis_subordinate()).get(key, setting)
        else:
            result = self.get_cache_backend()(get_redis_subordinate()).get_all(key)
        return result
...
def add_request(session, request_id, stub, system_date, stub_number,
    ...
    request_values = cache.get_cache_backend()(get_redis_subordinate()).values(cache.get_request_key(
        scenario_name))
